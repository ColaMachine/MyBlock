 ...
 */
public class LearnOpengMultiTexture {
...
// Setup an OpenGL context with API version 3.2
    ...
    public static ShaderConfig lightShaderConfig;
    public void initGL() throws IOException {

     /*   glClearColor(0, 0, 0, 0);
        glDisable(GL_DEPTH_TEST);
        glDisable(GL_LIGHTING);*/
         lightShaderConfig = new ShaderConfig("light", "multiTextureTest.frag", "multiTextureTest.vert", new int[]{3, 2,1,4});
        ...
        ShaderUtils.createProgram(lightShaderConfig);
        ShaderUtils.initUniform(lightShaderConfig);
        int textureHandle1= GLApp.makeTexture(PathManager.getInstance().getHomePath().resolve("assets/ui/chat.png").toString());
        int textureHandle2= GLApp.makeTexture(PathManager.getInstance().getHomePath().resolve("assets/ui/gui.png").toString());

        ShaderUtils.bindAndGetTextureIndex(lightShaderConfig,textureHandle1);
        ShaderUtils.bindAndGetTextureIndex(lightShaderConfig,textureHandle2);


        ShaderUtils.draw2dImg(lightShaderConfig,lightShaderConfig.getVao(),textureHandle1);


        TextureInfo textureInfo1 = new TextureInfo(0,0,11,11);
        textureInfo1.textureHandle= textureHandle1;

        TextureInfo textureInfo2 = new TextureInfo(0,0,11,11);
        textureInfo2.textureHandle= textureHandle1;




        ShaderUtils.draw2dImg(textureInfo1, 1/800,1/800,0+0.003f,256/800,256/800  , new Vector4f(0,0,0,0),lightShaderConfig);   OpenglUtils.checkGLError();
        ShaderUtils.draw2dImg(textureInfo2, 1/800,1/800,0+0.003f,256/800,256/800 , new Vector4f(0,0,0,0),lightShaderConfig);   OpenglUtils.checkGLError();

        lightShaderConfig.getVao().getVertices().rewind();
    ...
    void CreateVertShaders() throws IOException {
        ...
        String VertexShader = readShaderSourceCode( PathManager.getInstance().getInstallPath().resolve("src/main/java/gldemo/learnOpengl/"+shaderName+".vert").toString());
        //创建着色器
    ...
    String shaderName="multiTextureText";
    void CreateFragShaders() throws IOException {
        String FragmentShader = readShaderSourceCode( PathManager.getInstance().getInstallPath().resolve("src/main/java/gldemo/learnOpengl/"+shaderName+".frag").toString());
    ...
    public void render() throws LWJGLException {
        ...
        lightShaderConfig.getVao().getVertices().rewind();
        ShaderUtils.finalDraw(lightShaderConfig, lightShaderConfig.getVao());
    }


    public static void main(String[] args) {
        LWJGLHelper.initNativeLibs();//加载lib包
        LearnOpengMultiTexture main = null;
        try {
            main = new LearnOpengMultiTexture();
            main.create();
            main.run();
        } catch (Exception ex) {
            ex.printStackTrace();
