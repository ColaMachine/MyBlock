    ...
    public void moveOrAttack(LivingThingBean enemy) {
        ...
        if (length < 2) {//如果小于攻击距离
            ...
            if (TimeUtil.getNowMills() - enemy.getLastAttackTime() > 1000) {//如果攻击间隔已经够了
                ...
                AttackManager.addAttackEvent(enemy, enemy.getTarget());
                //enemy.attack();
...
*/                      //这段代码会导致一个问题 怪物到了目的地后才会再计算下一个目的地
            ...
            if (enemy.getFinalDest() == null) {//还没考虑好往哪里走呢
                //什么情况下 dest 为空  但是finalDest不为空  走到1个小旗帜的地方
                //什么情况下 dest 为空 finalDest也是空
                ...
                if (player != null) {
                    ...
                    //可达性分析
//                    GL_Vector gl_vector =new GL_Vector((float)Math.floor(enemy.getPosition().getX())+0.5f,
//                            (float)Math.floor(enemy.getPosition().getY())+0.5f,
//                            (float)Math.floor(enemy.getPosition().getZ())+0.5f
//                    );
//                    enemy.setDest(gl_vector);
                    //尝试直接走向目的地吧
                    LogUtil.println("尝试直接走向目的地吧");
                ...
                if (enemy.isBlock()) {//如果立马取消掉dest
                    ...
                    if (enemy.routes.size() == 0 && enemy.routes == null || enemy.routes.size() == 0) {
                        ...
                        LogUtil.err("有人更新了地理了吗 ");//正常路还没走完但是 被堵住了 选择归为 当当前方块中央位置
                        //是不是要清除 dest 和 finalDest
//                        enemy.setFinalDest(null);
//                        enemy.setDest(null);
                    ...
                    //如果当前有pathroute的结果  //如果没有被堵住 而且rotues 也走完了 //选择直接走向终点看看
                    if (enemy.getDest() == null && enemy.routes.size() == 0) {//走完了
                        enemy.setDest(enemy.getFinalDest());//尝试去
                    } else {//选择
                        //行走吧
