...
public class GroupBlock extends BaseBlock {
    ...
    public GroupBlock copy() {
        ...
        GroupBlock groupBlock = new GroupBlock(this.x, this.y, this.z, this.width, this.height, this.thick);
        groupBlock.id = id;

        groupBlock.live = this.live;
...
//    }
    ...
    public void shootBlock(float x, float y) {
        ...
        for (int i = 0; i < colorBlockList.size(); i++) {
            ...
            float[] info = this.getChildBlockPosition(block, this.x, this.y, this.z);
            AABB aabb = new AABB(new Vector3f(info[0] , info[1], info[2]),
                    new Vector3f(info[0] + info[3], info[1] + info[4], info[2] + info[5]));
                        ...
                        for (int k = 0; k < ary.length; k++) {
                            ...
                            if (ary[i] == ary2[k]) {
                                ...
                                try {
                                    ...
                                    float thisX = from.x + right[0] -this.x;

                                    float thisY = from.y + right[1] - this.y;
                                    float thisZ = from.z + right[2] -  this.z;
