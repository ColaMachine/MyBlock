...
//        GL11.glColor3ub(borderColor[0],borderColor[1],borderColor[2]);
    ...
    public static GL_Vector getLookAtDirection2(GL_Vector viewDir, float cursorX, float cursorY) {
        ...
        //newY/x= Math.tan(?);
        //newX/x= Math.tan(?);
        double x = (windowHeight / 2) / Math.tan(Math.PI / 4 / 2);
...
//        -0.5476552  -0.83405346
    ...
    public static GL_Vector getLookAtDirectionRotateMatrix(GL_Vector viewDir, float cursorX, float cursorY) {
        ...
        //newY/x= Math.tan(?);
        //newX/x= Math.tan(?);
        double x = (windowHeight / 2) / Math.tan(Math.PI / 4 / 2);//fov /2 时候对象的x轴的距离  也就是 视角的位置到 近端界面的距离
        newV.x = newX * -1;
        newV.y = newY;
        newV.z = (float) x;
        ...
        GL_Vector vectorAfter = viewDir.normalize().copyClone();
    ...
    public static GL_Vector getLookAtDirectionRotateNewMatrix(GL_Vector viewDir, float cursorX, float cursorY) {
        ...
        //newX/x= Math.tan(?);
        double x = (windowHeight / 2) / Math.tan(Math.PI / 4 / 2);//fov /2 时候对象的x轴的距离  也就是 视角的位置到 近端界面的距离
        GL_Vector source = GamingState.instance.camera.Position.copyClone();
        ...
        GL_Vector rightPoint = distanceEnd.copyClone().add(right.mult(newX));
        GL_Vector leftPoint = distanceEnd.copyClone().add(right.mult(-newX));

        GL_Vector upDir = viewDir.crossProduct(viewDir, right).normalize();

        GL_Vector upPoint = distanceEnd.copyClone().add(upDir.mult(-newY));
        GL_Vector downPoint = distanceEnd.copyClone().add(upDir.mult(newY));
        newV.x = newX * -1;
        newV.y = newY;
        newV.z = (float) x;
    ...
    public static GL_Vector getLookAtDirectionJiaodu(GL_Vector viewDir, float cursorX, float cursorY) {
        ...
        double zDistance = (windowHeight / 2) / Math.tan(Math.PI / 4 / 2);//fov /2 时候对象的x轴的距离  也就是 视角的位置到 近端界面的距离


        double yDegree = Math.atan(newY / zDistance);
        double xDegree = Math.atan(newX / zDistance);
        double ydu = yDegree /Constants.PIdiv180;
        double xdu = xDegree / Constants.PIdiv180;
        ...
        LogUtil.println("原始 xzDegree角度" + xzDegree / Constants.PIdiv180);

        float newXzDegree = (float) (Math.tan(xzDegree + xDegree) / Constants.PIdiv180 % 360);
      /*  if(newXzDegree<0){
            newXzDegree+=360;
        }*/
        LogUtil.println("选中后的 xzDegree角度" + newXzDegree);
        newV.z = (float) Math.tan(newXzDegree *Constants.PIdiv180) * newV.x;
