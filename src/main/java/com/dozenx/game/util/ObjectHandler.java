...
public class ObjectHandler {
	...
	private void setUp() {
		...
		glBindBuffer(GL_ARRAY_BUFFER, vbo);System.out.println("glEnableVertexAttribArray");
		glEnableVertexAttribArray(0);

		System.out.println("glVertexAttribPointer");
		...
		System.out.println("setUpShadowBuffer");
		if (shadow)
			setUpShadowBuffer();
	}
	...
	private void addVertices(Vertex[] vertices, int[] indices) {
		size = indices.length;
		System.out.println("assign vbo");
		glBindBuffer(GL_ARRAY_BUFFER, vbo);
		glBufferData(GL_ARRAY_BUFFER, createFlippedBuffer(vertices), GL_STATIC_DRAW);
		glBindBuffer(GL_ARRAY_BUFFER, 0);
		System.out.println("GL_ELEMENT_ARRAY_BUFFER");
		System.out.println("assign ibo");
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ibo);System.out.println("glBindBuffer(GL_ELEMENT_ARRAY_BUFFER");
	...
	public void setUpShadowBuffer() {System.out.println("");
		if (shadow) {
			glBindVertexArray(abo);
	...
	public void updateShadowBuffer(int shadowShaderProgram, int depthModelMatrixID) {
		if (shadow) {
			...
			Util.checkGLError();
			modelMatrix.store(matrix44Buffer);
			matrix44Buffer.flip();Util.checkGLError();
			glUniformMatrix4(depthModelMatrixID, false, matrix44Buffer);Util.checkGLError();
			System.out.println("glDrawElements");
			glBindVertexArray(abo);Util.checkGLError();
			glDrawElements(GL_TRIANGLES, size, GL_UNSIGNED_INT, 0);Util.checkGLError();
			glBindVertexArray(0);Util.checkGLError();
			System.out.println("glDrawElements success !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
			glUseProgram(0);
		}
	}
