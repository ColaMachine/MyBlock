			...
			if(this.selectedIndex>0){
				 TextureInfo ti = TextureManager.getIcon("selectBox");
		            ...
		            GL11.glBegin(GL11.GL_QUADS);
		            {
		                ...
		                GL11.glTexCoord2f(ti.minX, ti.minY);
		                GL11.glVertex3f( minX+(selectedIndex-1)*this.getWidth()/9, minY, 0);

		                GL11.glTexCoord2f(ti.maxX,ti. minY);
		                GL11.glVertex3f( minX+(selectedIndex)*this.getWidth()/9+5, minY, 0);

		                GL11.glTexCoord2f(ti.maxX, ti.maxY);
		                GL11.glVertex3f( minX+(selectedIndex)*this.getWidth()/9+5, maxY+5.2f,0);

		                GL11.glTexCoord2f(ti.minX, ti.maxY);
		                GL11.glVertex3f(minX+(selectedIndex-1)*this.getWidth()/9,maxY+5.2f, 0);
