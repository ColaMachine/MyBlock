...
public final class WorldRendererLwjgl implements WorldRenderer {
    ...
    private static final int MAX_CHUNKS = ViewDistance.MEGA.getChunkDistance() * ViewDistance.NEAR.getChunkDistance();
    ...
    /* CHUNKS */
    private ChunkTessellator chunkTessellator;//块的曲面细分
    private boolean pendingChunks;//待定方块
    private final List<ChunkImpl> chunksInProximity = Lists.newArrayListWithCapacity(MAX_CHUNKS);//接近的方块
    private int chunkPosX;
    private int chunkPosZ;

    /* RENDERING *///不透明物体  不透明物体阴影  透明反射体 透明混色体
    ...
    public void render(DefaultRenderingProcess.StereoRenderState stereoRenderState) {
        ...
        glDepthMask(false);//可将深度缓冲区设置为只读形式

        /*for (RenderSystem renderer : systemManager.iterateRenderSubscribers()) {
            renderer.renderAlphaBlend();//LocalPlayerSystem--BlockParticleEmitterSystem@3f784xxx---BlockSelectionRenderSystem xxx MeshRenderer---FirstPersonRenderer--BlockDamageRenderer--SkeletonRenderer
        }*/
    ...
    public void renderWorld(Camera camera) {//开始绘画世界
      ...
      /*  PerformanceMonitor.startActivity("Render Sky");
        ...
        skysphere.render(camera);//zzw 天空渲染
        ...
        for (RenderSystem renderer : systemManager.iterateRenderSubscribers()) {//掉落在地上的物品
       ...
       /**/ PerformanceMonitor.startActivity("Render Chunks (Opaque)");
        while (renderQueueChunksOpaque.size() > 0) {
           renderChunk(renderQueueChunksOpaque.poll(), ChunkMesh.RenderPhase.OPAQUE, camera, ChunkRenderMode.DEFAULT);//zzw 块状render
       }
        PerformanceMonitor.endActivity();

        /*
         * SECOND RENDER PASS: ALPHA REJECT //zzw花花草草
         */
	...
	*/
        ...
        */
         PerformanceMonitor.startActivity("Render Chunks (Refractive/Reflective)");//什么都没做啊
        ...
        while (renderQueueChunksAlphaBlend.size() > 0) { //ZZW 渲染水部分 
    ...
    private void renderChunk(ChunkImpl chunk, ChunkMesh.RenderPhase phase, Camera camera, ChunkRenderMode mode) {//渲染水的主要部分
        if (chunk.getChunkState() == ChunkImpl.State.COMPLETE && chunk.getMesh() != null) {//判断chunk是否完成了 判断chunk是否有mesh材质
            Material shader = null;

            final Vector3f cameraPosition = camera.getPosition(); //摄像机位置
            final Vector3f chunkPositionRelToCamera =
                    new Vector3f(chunk.getPos().x * ChunkConstants.SIZE_X - cameraPosition.x,
                            ...
                            chunk.getPos().z * ChunkConstants.SIZE_Z - cameraPosition.z);//chunk相对于相机的位置

            if (mode == ChunkRenderMode.DEFAULT || mode == ChunkRenderMode.REFLECTION) {
                shader = Assets.getMaterial("engine:prog.chunk");//块状的材料
               // shader.enable();//开启了什么
            ...
            for (int i = 0; i < VERTICAL_SEGMENTS; i++) {
                if (!chunk.getMesh()[i].isEmpty()) {
                    ...
                    chunk.getMesh()[i].render(phase);//zzw 具体的方块绘画
